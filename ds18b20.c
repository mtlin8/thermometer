...
void ds_write1bit(void)
{
    ...
    // Pull bus low for 60 microseconds
    DDRC |= (1 << PC3);         // Pull bus low
    _delay_us(2);             // Delay >480usec

    DDRC &= ~(1 << PC3);        // Let bus go high
    _delay_us(60);              // Wait for Presense pulse
...
void ds_write0bit(void)
{
    ...
    // Pull bus low for 2 microseconds
    DDRC |= (1 << PC3);         // Pull bus low
    _delay_us(60);             // Delay >480usec

    DDRC &= ~(1 << PC3);        // Let bus go high
    _delay_us(2);              // Wait for Presense pulse
}
...
unsigned char ds_readbit(void)
{
    ...
    // Will return 0 if the bit exists.
    DDRC |= (1 << PC3);         // Pull bus low
    _delay_us(2);             // Delay > 480 usec
    DDRC &= ~(1 << PC3);        // Let bus go high
    _delay_us(10);
    char x = (PINC & (1 << PC3));
    _delay_us(50);
    return(x); // either zero or nonzero no need to get tripped up
}
